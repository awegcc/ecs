...
viprexec -i -c "zgrep candidateCount /var/log/blobsvc-chunk-reclaim.log${datelist}" > candidateCount.log.${datenow}
viprexec -i -c "zgrep This /var/log/blobsvc-chunk-reclaim.log${datelist}" > roundEnd.log.${datenow}
...
do
    ...
    lastEndtime=$(grep ${rg}_${ob}  roundEnd.log.${datenow} | grep REPO_SCAN | grep round | tail -n 1 | awk -F '2017-' '{print $2}' | awk -F ',' '{print $1}')
