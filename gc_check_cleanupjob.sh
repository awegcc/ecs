...
admin@provo-cornsilk:~> curl http://`hostname`:9101/diagnostic/OB/0/ | xmllint --format - | grep table_detail_link | awk -F"[<|>|?]" '{print "echo \""$3"\"\n""curl -L \""$3"DELETE_JOB_TABLE_KEY?type=CLEANUP_JOB&objectId=aa&maxkeys=1&useStyle=raw\""}' > /tmp/cleanup_job_command.sh

# Execute the generated script

admin@provo-cornsilk:~> sh /tmp/cleanup_job_command.sh > /tmp/cleanupjob.tmp

# Find the earliest expire time for fetched cleanup jobs

admin@provo-cornsilk:~> grep schemaType /tmp/cleanupjob.tmp | awk '{print $4}' | sort -n | head -3
1479361654707
1479361973815
1479362655633
...
admin@provo-cornsilk:~> curl -L "http://10.247.195.130:9101/urn:storageos:OwnershipInfo:15519928-e6ea-4688-b9fe-813c86104730_6599ff72-89cf-4daa-aa91-d8b96196293f_OB_73_128_0:/DELETE_JOB_TABLE_KEY?type=CLEANUP_JOB&objectId=aa&maxkeys=1&useStyle=raw" | grep schema
schemaType DELETE_JOB_TABLE_KEY expireTime 1479361654707 type CLEANUP_JOB objectIndexKey schemaType OBJECT_TABLE_KEY objectId df2a2a3e0dccd7134e8057d398e572b338cff189954ad50e722dc8f3d579f22f type UPDATE sequence 0 keyVersion version22 indexSequence 1 versionId 0
